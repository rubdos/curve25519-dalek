...
//! `avx2` module.
//!
//! Similar to field, code for this was mostly copied from avx2 module.
...
impl ExtendedPoint {
    ...
    pub fn double(&self) -> ExtendedPoint {
        ...
        // tmp0 = (0, 0,  2S3, -S4)
        tmp0 = tmp0 + S_1;
        // tmp0 = (  S1,   S1, S1 + 2S3, S1 - S4)
        tmp0 = tmp0 + zero.blend(S_2, Lanes::AD);
        // tmp0 = (S1 + S2,   S1, S1 + 2S3, S1 + S2 - S4)
        tmp0 = tmp0 + zero.blend(S_2.negate_lazy(), Lanes::BC);
        // tmp0 = (S1 + S2, S1 - S2, S1 - S2 + 2S3, S1 + S2 - S4)
        //    b < (     1.01,       1.6,             2.33,             1.6)
        // Now tmp0 = (S5, S6, S8, S9)

        // Set tmp1 = ( S9,  S6,  S6,  S9)
        //        b < ( 1.6,  1.6,  1.6,  1.6)
        tmp1 = tmp0.shuffle(Shuffle::DBBD);
        // Set tmp0 = ( S8,  S5,  S8,  S5)
...
mod test {
    ...
    fn basepoint_odd_lookup_table_verify() {
        ...
        for (b_vec, base_vec) in table_B.0.iter().zip(basepoint_odd_table.0.iter()) {
            ...
            println!("{:?}", base_splits[0]);
            println!("{:?}", base_splits[1]);
            println!("{:?}", base_splits[2]);
            println!("{:?}", base_splits[3]);
            println!("----");
            println!("{:?}", b_splits[0]);
            println!("{:?}", b_splits[1]);
            println!("{:?}", b_splits[2]);
